      ...
      <button onClick={() => setOuvert((o) => !o)} className="w-full text-left px-4 py-3 flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {boite.en_alerte && (
              <span title="Seuil d'alerte atteint" className="text-[var(--statut-rupture)]">
          ...
          <div className="mt-1.5 h-1.5 w-full max-w-xs rounded-full bg-black/5 dark:bg-white/10 overflow-hidden">
            <div
              className={`h-full rounded-full ${boite.en_alerte ? "bg-[var(--statut-rupture)]" : "bg-[var(--accent)]"}`}
        ...
        <div className="px-4 pb-4 space-y-4 border-t border-[var(--hairline)] pt-3">
          ...
          {peutModifier && (
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-[var(--hairline)]">
              ...
              <button
                onClick={supprimer}
                className="inline-flex items-center gap-1.5 text-xs font-medium text-[var(--statut-rupture)] hover:underline"
              >
